...
interface Preferences {theme:string;large:boolean;language:UiLanguage}
export function usePreferences(onError:(text:string)=>void){
 const defaultLanguage:UiLanguage=typeof navigator!=='undefined'&&!navigator.language.toLowerCase().startsWith('ja')?'en':'ja';
 const [value,setValue]=useState<Preferences>({theme:'plain',large:false,language:defaultLanguage}),[ready,setReady]=useState(false);
 const store=useRef(new PlatformStorage()).current,pending=useRef<Promise<unknown>>(Promise.resolve());
 useEffect(()=>{let alive=true;void store.get('user-preferences').then(bytes=>{if(!alive)return;if(bytes){const p=decodeJson(bytes) as Partial<Preferences>;if(appThemes.some(t=>t.id===p.theme)&&typeof p.large==='boolean')setValue({theme:p.theme!,large:p.large,language:p.language==='en'?'en':'ja'});}setReady(true);}).catch(()=>{if(alive){onError('画面設定を読み取れませんでした。初期設定で表示します。');setReady(true);}});return()=>{alive=false;};},[store]);
...
export function PreferencesDialog({preferences,onClose}:{preferences:ReturnType<typeof usePreferences>;onClose:()=>void}){
 ...
 return <Modal title="設定・使い方" onClose={onClose}>
   <h3>表示言語</h3><label className="field"><select aria-label="表示言語" disabled={!preferences.ready} value={value.language} onChange={e=>set({...value,language:e.target.value as UiLanguage})}><option value="ja">日本語</option><option value="en">English</option></select></label>
   ...
   <fieldset className="theme-picker" disabled={!preferences.ready}><legend>画面の色を選ぶ</legend><div className="theme-grid">{appThemes.map(theme=><button type="button" key={theme.id} aria-pressed={value.theme===theme.id} onClick={()=>set({...value,theme:theme.id})}><span className="theme-preview" style={{backgroundColor:theme.soft}}><span style={{backgroundColor:theme.accent}}/></span><span>{theme.name}</span></button>)}</div></fieldset><p className="help-text">色見本を押すと、操作画面の配色だけが変わります。便箋の絵柄や印刷内容には影響しません。</p><hr/><h3>困ったときは</h3><ul className="guide-list"><li>保存されたか不安なときは、画面右上の保存状態を確認します。「ファイル」から今すぐ保存もできます。</li><li>写真を別ページへ動かすときは、写真を選んで「配置方法」を「ページの位置へ固定」にすると配置ページを選べます。写真をページの端へドラッグし続けても移動できます。</li><li>印刷前に「PDF・印刷」のプレビューで、用紙の端やページを確認します。</li></ul><details><summary>キーボードでの便利な操作</summary><p>Ctrl + S：保存 ／ Ctrl + Z：元に戻す<br/>Ctrl + Enter：改ページ ／ Ctrl + A：本文を全選択</p></details><hr/><SoftwareLicenses/><FontLicenses/><p className="help-text">レタリエ {APP_VERSION} — 想いを、一枚の手紙に。<br/>© 2026 Y-TEC · Apache License 2.0<br/>インターネット接続は不要です。画像・手紙はPC内で扱います。</p></Modal>;
}
