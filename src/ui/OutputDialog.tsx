...
export function OutputDialog({project,missing,onClose}:{project:Project;missing:string[];onClose:()=>void}){
 ...
 const run=async(kind:'pdf'|'print')=>{
  ...
  try{
   ...
   // Large photos used to be decoded one by one here.  Decode every asset at once:
   // the print compositor still waits for all of them, without adding a full load
   // time for each individual image.
   await Promise.all(Array.from(document.querySelectorAll<HTMLImageElement>('.print-output img')).map(img=>img.complete?Promise.resolve():img.decode()));
