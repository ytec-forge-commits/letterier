...
import {decorationMotifPlacement} from '../core/stationery-layout';
import {templates} from '../core/templates';
export {decorationMotifPlacement} from '../core/stationery-layout';
...
export function PaperDecoration({design,width,height,writingMode}:{design:string;width:number;height:number;writingMode:WritingMode}){
  ...
  const placement=decorationMotifPlacement(width,height,writingMode,continuation);
  const rotate=writingMode==='horizontal';
  return <svg className="paper-decoration" width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} aria-hidden="true">
    {artPath&&<image href={artPath} x={placement.x} y={placement.y} width={placement.width} height={placement.height} opacity={continuation?.58:.82} preserveAspectRatio="xMinYMin meet" transform={rotate?`rotate(180 ${placement.x+placement.width/2} ${placement.y+placement.height/2})`:undefined}/>}
