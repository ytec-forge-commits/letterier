...
import { changeWritingMode } from '../core/objects';
...
export function App() {
  ...
  const appTheme=appThemes.find(theme=>theme.id===preferences.value.theme)??appThemes[0];
  ...
  const change=(next:Project,restore=true)=>{next=pruneAssets(next);try{const total=Object.values(next.assets).reduce((sum,a)=>sum+a.data.length+(a.original?.data.length??0),0);if(total>64*1024*1024*4/3)throw Error('素材の合計が64MBを超えます。不要な画像を削除するか、別の手紙に分けてください。');assertImageBudget(next.assets);compose(next,measureText);}catch(e){setError(String(e));return false;}past.current.push(project);trimUndo(past.current,next);future.current=[];setProject(next);if(restore)setCaret(v=>v+1);return true;};
  const changeObject=(id:string,patch:Partial<FloatingObject>)=>{const ok=change({...project,objects:project.objects.map(o=>o.id===id?{...o,...patch}:o)},false);if(ok&&patch.style?.fontFamily&&patch.style.fontFamily!==project.objects.find(o=>o.id===id)?.style?.fontFamily)fonts.record(patch.style.fontFamily);return ok;};
  ...
  const undo=(redo=false)=>{
    ...
    const p=source.pop();if(!p)return;target.push(project);setTyping(null);setProject(p);setSelection(s=>({start:Math.min(s.start,bodyText(p).length),end:Math.min(s.end,bodyText(p).length)}));setCaret(v=>v+1);
  };
  const insert=(start:number,end:number,text:string)=>{try{const normalized=text.replace(/\r\n?/g,'\n').replace(/\t/g,'　'),style=typing?.at===start?typing.style:undefined;if(!change(replaceRange(project,start,end,normalized,style)))return;const at=start+normalized.length;setSelection({start:at,end:at});if(style){setTyping({at,style});if(normalized&&style.fontFamily)fonts.record(style.fontFamily);}}catch(e){setError(String(e));}};
  ...
  return <><div className="app-shell" inert={files.busy||imageBusy||!files.ready} onKeyDown={e=>{if(e.ctrlKey&&!e.nativeEvent.isComposing){if(e.key.toLowerCase()==='s'){e.preventDefault();void(e.shiftKey?files.saveAs():files.saveNow());}if(e.key.toLowerCase()==='o'){e.preventDefault();void files.open();}}}}>
    <header className="app-header"><div className="brand"><span className="brand-mark">✒</span><div><strong>レタリエ</strong><small>想いを、一枚の手紙に。</small></div></div><input className="document-title" aria-label="手紙の名前" maxLength={200} value={project.title} onChange={e=>change({...project,title:e.target.value},false)}/>{appTheme.previewDesign&&<span className="theme-header-art"><PaperDecoration design={appTheme.previewDesign} width={88} height={54} writingMode="horizontal"/></span>}<span className="save-status" role="status">{files.status}</span><button className="primary" onClick={()=>void files.run(async()=>{await files.flush();setDialog('output');})}>PDF・印刷</button></header>
    <nav className="toolbar" aria-label="編集操作"><button onClick={()=>setDialog('files')}>ファイル</button><button onClick={()=>setDialog('templates')}>便箋を選ぶ</button><button onClick={()=>void files.run(async()=>{await files.flush();await files.repository.snapshot(project);setDialog('history');})}>保存履歴</button><span className="divider"/><button aria-label="元に戻す" disabled={!past.current.length} onMouseDown={e=>e.preventDefault()} onClick={()=>undo()}>↶ 元に戻す</button><button aria-label="やり直す" disabled={!future.current.length} onMouseDown={e=>e.preventDefault()} onClick={()=>undo(true)}>↷ やり直す</button><span className="divider"/><button onClick={()=>imageInput.current?.click()}>写真・画像</button><button onClick={()=>void addObject('text')}>文字箱</button><input ref={imageInput} type="file" accept={imageAccept} hidden onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file)void addObject('image',file);}}/><button className="settings-button" onClick={()=>setDialog('preferences')}>画面設定・使い方</button></nav>
    <div className="workbench">
      ...
      <div className="canvas-area"><div className="canvas-bar"><span>{project.settings.paper==='POSTCARD'?'はがき':project.settings.paper} · {project.settings.writingMode==='vertical'?'縦書き':'横書き'}</span><label>表示 <select aria-label="表示倍率" value={zoom} onChange={e=>setZoom(Number(e.target.value))}><option value="0.5">50%</option><option value="0.65">65%</option><option value="0.8">80%</option><option value="1">100%</option><option value="1.25">125%</option></select></label></div>
        <EditorCanvas currentStyle={currentStyle} project={renderProject} layout={layout} zoom={zoom} selection={selection} caretRequest={caret} onSelection={s=>{if(s.start!==selection.start||s.end!==selection.end)setTyping(null);setSelection(s);}} onInsert={insert} onHistory={undo} onFormat={format} onPage={p=>{setPage(p);setSelectedObject(null);}} selectedObject={selectedObject} onSelectObject={id=>{setSelectedObject(id);setPage(layout.objects.find(o=>o.id===id)?.actualPage??0);}} onObjectPreview={(id,patch)=>setObjectPreview(patch?{id,patch}:null)} onObjectChange={changeObject} onObjectDelete={deleteObject}/>
      </div>
      <aside className="right-panel">{activeObject?<ObjectProperties key={activeObject.id} object={activeObject} project={project} pageCount={layout.pages.length} fonts={fonts.fonts} recentFonts={fonts.recent} onChange={patch=>changeObject(activeObject.id,patch)} onDelete={()=>deleteObject(activeObject.id)} onDuplicate={()=>{if(project.objects.length>=200){setError('自由配置の要素は200個までです。');return;}const id=uid();change({...project,objects:[...project.objects,{...project.objects.find(o=>o.id===activeObject.id)!,id,x:activeObject.x+5,y:activeObject.y+5}]},false);setSelectedObject(id);}} onMode={changeAnchor} onClose={()=>setSelectedObject(null)}/>:<><div className="panel-title">文字の書式</div><p className="help-text">一部の文字を選ぶと、その部分だけ変更できます。</p><div className="field"><label htmlFor="font">フォント</label><FontSelect id="font" value={currentStyle.fontFamily} onChange={fontFamily=>format({fontFamily})} fonts={fonts.fonts} recent={fonts.recent}/></div>
  ...
  {dialog==='recent'&&<Modal title="最近使った手紙" onClose={()=>setDialog(null)}><div className="recent-list">{recent.length===0&&<p>まだ手紙がありません。</p>}{recent.map(d=><button key={d.id} onClick={()=>{setDialog(null);void files.openRecent(d.id);}}><strong>{d.title||'名前のない手紙'}</strong><small>{new Date(d.time).toLocaleString('ja-JP')}</small></button>)}</div></Modal>}
  ...
  {dialog==='templates'&&<TemplateDialog project={project} onClose={()=>setDialog(null)} onNew={files.createFrom} onApply={p=>change(p,false)}/>}
  {dialog==='output'&&<OutputDialog project={project} missing={missing} onClose={()=>setDialog(null)}/>}
  {dialog==='preferences'&&<PreferencesDialog preferences={preferences} onClose={()=>setDialog(null)}/>}
