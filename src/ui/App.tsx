...
export function App() {
  ...
  return <><div className="app-shell" inert={files.busy||imageBusy||!files.ready} onKeyDown={e=>{if(e.ctrlKey&&!e.nativeEvent.isComposing){if(e.key.toLowerCase()==='s'){e.preventDefault();void(e.shiftKey?files.saveAs():files.saveNow());}if(e.key.toLowerCase()==='o'){e.preventDefault();void files.open();}}}}>
    <header className="app-header"><div className="brand"><span className="brand-mark" aria-hidden="true"><span className="brand-nib">✦</span></span><div><strong>レタリエ</strong><small>想いを、一枚の手紙に。</small></div></div><input className="document-title" name="document-title" autoComplete="off" aria-label="手紙の名前" maxLength={200} value={project.title} onChange={e=>change({...project,title:e.target.value},false)}/><nav className="header-tools" aria-label="編集操作"><button onClick={()=>setDialog('files')}>ファイル</button><button onClick={()=>setDialog('templates')}>便箋を選ぶ</button><button onClick={()=>void files.run(async()=>{await files.flush();await files.repository.snapshot(project);setDialog('history');})}>保存履歴</button><span className="divider"/><button disabled={!past.current.length} onMouseDown={e=>e.preventDefault()} onClick={()=>undo()}>↶ 元に戻す</button><button disabled={!future.current.length} onMouseDown={e=>e.preventDefault()} onClick={()=>undo(true)}>↷ やり直す</button><span className="divider"/><button onClick={()=>imageInput.current?.click()}>写真・画像</button><button onClick={()=>void addObject('text')}>文字箱</button><button className="settings-button" onClick={()=>setDialog('preferences')}>設定・使い方</button></nav><input ref={imageInput} type="file" accept={imageAccept} hidden onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file)void addObject('image',file);}}/>{appTheme.previewDesign&&<span className="theme-header-art"><PaperDecoration design={appTheme.previewDesign} width={72} height={48} writingMode="horizontal"/></span>}<span className="save-status" role="status" aria-live="polite">{files.status}</span><button className="primary" onClick={()=>void files.run(async()=>{await files.flush();setDialog('output');})}>PDF・印刷</button></header>
