...
export function ObjectLayer(props:Props){
  ...
  const end=(e:PointerEvent<HTMLElement>)=>{const d=drag.current;if(!d)return;if(d.action==='move')updateMove(e.clientX,e.clientY,false);cancelAnimationFrame(d.raf);drag.current=null;setGhost(null);props.onPreview(d.object.id,null);if(Object.keys(d.patch).length)props.onChange(d.object.id,d.patch);};
  const cancel=()=>{if(drag.current){cancelAnimationFrame(drag.current.raf);props.onPreview(drag.current.object.id,null);}drag.current=null;setGhost(null);};
